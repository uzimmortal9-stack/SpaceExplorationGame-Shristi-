...
| **Walking through interior walls** | Widening the spine to close the floor gap made the corridor *walkable* on the room side, and the wall rule only emitted a wall where the neighbour was **not** walkable — so every room's spine-facing partition (and its collider) silently vanished. | A ROOM boundary now always emits a wall, minus doorways, independent of what is on the other side. Corridor-to-corridor seams stay open. `tools/sim_walls.py` asserts both invariants and fails if either regresses. |
